...
export default function RoomCard({ room, currentUserId, userRole, toggleFavorite, onEdit, onDelete, onChat }) {
  ...
  const roomAmenities = AMENITIES.filter(a => room.amenities.includes(a.key)).map(a => a.label).join(', ');
      ...
      <div className="md:w-2/3 p-4 flex flex-col justify-between">
        <div>
          ...
          <div className="space-y-1 text-sm text-gray-600">
            <p className="flex items-center"><MapPin className="w-4 h-4 mr-2 text-indigo-400" /> {room.location || room.address}</p>
            <p className="flex items-center"><Ruler className="w-4 h-4 mr-2 text-indigo-400" /> {room.area} m²</p>
            <p className="flex items-center"><Zap className="w-4 h-4 mr-2 text-indigo-400" /> Tiện ích: {roomAmenities || 'Không có'}</p>
          </div>
        </div>
